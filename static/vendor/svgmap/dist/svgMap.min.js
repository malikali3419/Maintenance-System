...
svgMap.prototype.init = function(l) {
        ...
        ZM: "Zambia",
        ZW: "Zimbabwe"
    }, svgMap.prototype.applyData = function(t) {
        var a = null,
            o = null;
        Object.keys(t.values).forEach(function(l) {
            l = parseInt(t.values[l][t.applyData], 10);
            (a = null === a ? l : a) < l && (a = l), l < (o = null === o ? l : o) && (o = l)
        }), t.data[t.applyData].thresholdMax && (a = Math.min(a, t.data[t.applyData].thresholdMax)), t.data[t.applyData].thresholdMin && (o = Math.max(o, t.data[t.applyData].thresholdMin)), Object.keys(this.countries).forEach(function(l) {
            var L = document.getElementById(this.id + "-map-country-" + l);
            L && (t.values[l] ? (l = Math.max(o, parseInt(t.values[l][t.applyData], 10)), l = Math.max(0, Math.min(1, (l - o) / (a - o))), l = this.getColor(this.options.colorMax, this.options.colorMin, l), L.setAttribute("fill", l)) : L.setAttribute("fill", this.options.colorNoData))
        }.bind(this))
        ...
        var l = this.createElement("div", "svgMap-map-controls-wrapper", this.mapWrapper),
            t = this.createElement("div", "svgMap-map-controls-zoom", l);
        ["in", "out"].forEach(function(l) {
            var L = "zoomControl" + l.charAt(0).toUpperCase() + l.slice(1);
            this[L] = this.createElement("button", "svgMap-control-button svgMap-zoom-button svgMap-zoom-" + l + "-button", t), this[L].type = "button", this[L].addEventListener("click", function() {
                this.zoomMap(l)
            }.bind(this))
        ...
        this.options.countries.EH || (l.MA.d = l["MA-EH"].d, delete l.EH), delete l["MA-EH"], Object.keys(l).forEach(function(l) {
            var t, L = this.mapPaths[l];
            L.d && ((t = document.createElementNS("http://www.w3.org/2000/svg", "path")).setAttribute("d", L.d), t.setAttribute("id", this.id + "-map-country-" + l), t.setAttribute("data-id", l), t.classList.add("svgMap-country"), this.mapImage.appendChild(t), ["mouseenter", "touchdown"].forEach(function(l) {
                t.addEventListener(l, function() {
                    t.closest("g").appendChild(t)
                }.bind(this))
            }.bind(this)), t.addEventListener("touchstart", function(l) {
                var L = t.getAttribute("data-id");
                this.setTooltipContent(this.getTooltipContent(L)), this.showTooltip(l), this.moveTooltip(l)
            }.bind(this)), t.addEventListener("mouseenter", function(l) {
                var L = t.getAttribute("data-id");
                this.setTooltipContent(this.getTooltipContent(L)), this.showTooltip(l)
            }.bind(this)), t.addEventListener("mousemove", function(l) {
                this.moveTooltip(l)
        ...
        this.mapPanZoom = svgPanZoom(this.mapImage, {
            ...
            onZoom: function() {
                M.setControlStatuses()
            },
            beforePan: function(l, L) {
                var t = .85 * M.mapWrapper.offsetWidth,
                    a = .85 * M.mapWrapper.offsetHeight,
                    o = this.getSizes(),
                    e = -((o.viewBox.x + o.viewBox.width) * o.realZoom) + t,
                    i = o.width - t - o.viewBox.x * o.realZoom,
                    t = -((o.viewBox.y + o.viewBox.height) * o.realZoom) + a,
                    o = o.height - a - o.viewBox.y * o.realZoom;
                return {
                    x: Math.max(e, Math.min(i, L.x)),
                    y: Math.max(t, Math.min(o, L.y))
                }
            }
        }), this.mapPanZoom.zoom(this.options.initialZoom), this.setControlStatuses()
    }, svgMap.prototype.getTooltipContent = function(t) {
        var l = this.createElement("div", "svgMap-tooltip-content-container");
        !1 === this.options.hideFlag && (L = this.createElement("div", "svgMap-tooltip-flag-container svgMap-tooltip-flag-container-" + this.options.flagType, l), "image" === this.options.flagType ? this.createElement("img", "svgMap-tooltip-flag", L).setAttribute("src", this.options.flagURL.replace("{0}", t.toLowerCase())) : "emoji" === this.options.flagType && (L.innerHTML = this.emojiFlags[t])), this.createElement("div", "svgMap-tooltip-title", l).innerHTML = this.getCountryName(t);
        var a, L = this.createElement("div", "svgMap-tooltip-content", l);
        return this.options.data.values[t] ? (a = "<table>", Object.keys(this.options.data.data).forEach(function(l) {
            var L = this.options.data.data[l],
                l = this.options.data.values[t][l];
            L.floatingNumbers && (l = l.toFixed(1)), L.thousandSeparator && (l = this.numberWithCommas(l, L.thousandSeparator)), l = L.format ? L.format.replace("{0}", "<span>" + l + "</span>") : "<span>" + l + "</span>", a += "<tr><td>" + (L.name || "") + "</td><td>" + l + "</td></tr>"
        }.bind(this)), a += "</table>", L.innerHTML = a) : this.createElement("div", "svgMap-tooltip-no-data", L).innerHTML = this.options.noDataText, l
        ...
        var L, t, a, o, e = l.pageX || (l.touches && l.touches[0] ? l.touches[0].pageX : null),
            i = l.pageY || (l.touches && l.touches[0] ? l.touches[0].pageY : null);
        null !== e && null !== i && (L = window.innerWidth, t = this.tooltip.offsetWidth, a = this.tooltip.offsetHeight, (o = e - t / 2) <= 6 ? (e = 6 + t / 2, this.tooltipPointer.style.marginLeft = o - 6 + "px") : L - 6 <= o + t ? (e = L - 6 - t / 2, this.tooltipPointer.style.marginLeft = -1 * (L - 6 - l.pageX - t / 2) + "px") : this.tooltipPointer.style.marginLeft = "0px", i - 12 - a <= 6 ? (this.tooltip.classList.add("svgMap-tooltip-flipped"), i += 32) : (this.tooltip.classList.remove("svgMap-tooltip-flipped"), i -= 12), this.tooltip.style.left = e + "px", this.tooltip.style.top = i + "px")
    }, svgMap.prototype.error = function(l) {
        (console.error || console.log)("svgMap error: " + (l || "Unknown error"))
    }, svgMap.prototype.createElement = function(l, L, t, a) {
        var o = document.createElement(l);
        return L && (L = L.split(" ")).forEach(function(l) {
            o.classList.add(l)
        }), a && (o.innerHTML = a), t && t.appendChild(o), o
    }, svgMap.prototype.numberWithCommas = function(l, L) {
        return l.toString().replace(/\B(?=(\d{3})+(?!\d))/g, L || ",")
    }, svgMap.prototype.getColor = function(l, L, t) {
        l = l.slice(-6), L = L.slice(-6);
        var a = Math.ceil(parseInt(l.substring(0, 2), 16) * t + parseInt(L.substring(0, 2), 16) * (1 - t)),
            o = Math.ceil(parseInt(l.substring(2, 4), 16) * t + parseInt(L.substring(2, 4), 16) * (1 - t)),
            t = Math.ceil(parseInt(l.substring(4, 6), 16) * t + parseInt(L.substring(4, 6), 16) * (1 - t));
        return "#" + this.getHex(a) + this.getHex(o) + this.getHex(t)
